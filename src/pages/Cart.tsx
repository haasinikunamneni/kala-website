...
export function Cart() {
  ...
  const artworkLines = items
    .map(
      (i) =>
        `- ${i.painting.title} (₹${i.painting.price.toLocaleString("en-IN")})${
          i.quantity > 1 ? ` x${i.quantity}` : ""
        }`
    )
    .join("\n");

  const enquiryMessage = `Hey kalā !!\nI wanted to enquire about one of your collections.  ⭐️\nMy name :\nArtwork name:\n${artworkLines}\nCity:\nPin code:`;
    ...
    <div className="px-6 pb-24 pt-32 md:px-10">
      <div className="mx-auto max-w-5xl">
          ...
          {items.map((i) => {
            ...
            return (
              <div key={`${i.painting.id}-${i.frame}-${i.glass}`} className="flex gap-5 py-7">
                <Link to={`/artwork/${i.painting.slug}`} className="h-24 w-20 flex-shrink-0 overflow-hidden rounded-sm bg-charcoal/5">
        ...
        <div className="mt-10 rounded-sm border border-charcoal/10 bg-beige px-6 py-6">
          <p className="font-display text-lg text-charcoal">To place an order, simply message us on Whatsapp or Instagram:</p>
          <p className="mt-4 font-body text-sm leading-relaxed text-charcoal/75">
            Hey kalā !!<br />
            I wanted to enquire about one of your collections.  ⭐️
          </p>
          <ul className="mt-4 space-y-1.5 font-body text-sm text-charcoal/75">
            <li><span className="text-charcoal/50">* My name :</span> <span className="italic text-charcoal/40">to be filled by you</span></li>
            <li>
              <span className="text-charcoal/50">* Artwork name:</span>{" "}
              <span className="text-charcoal/75">
                {items.map((i) => `${i.painting.title} (₹${i.painting.price.toLocaleString("en-IN")})`).join(", ")}
              </span>
            </li>
            <li><span className="text-charcoal/50">* City:</span> <span className="italic text-charcoal/40">to be filled by you</span></li>
            <li><span className="text-charcoal/50">* Pin code:</span> <span className="italic text-charcoal/40">to be filled by you</span></li>
          </ul>
